...
export default function RegisterPage({ onRegister, onSwitchToLogin }: {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      if (Array.isArray(detail)) setError(detail[0]?.msg || 'Could not create account.')
      else setError(detail || 'Could not create account. Try a different username.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-surface flex items-center justify-center p-4">
      ...
      <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:0.4}}
        ...
        <div className="card">
          <p className="text-xs text-muted mb-1 text-center">Create your private workspace</p>
          <p className="flex items-center justify-center gap-1.5 text-[11px] text-brand mb-5 text-center">
            <ShieldCheck className="w-3.5 h-3.5"/> Your inspections are visible only to you
          </p>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input value={username} onChange={e=>setUsername(e.target.value)}
                  ...
                  placeholder="jane.doe" minLength={3} maxLength={32} required/>
              </div>
            </div>
            <div>
          ...
          <p className="text-center text-xs text-muted mt-4">
            ...
            <button onClick={onSwitchToLogin} className="text-brand hover:underline">Sign in</button>
          </p>
        </div>
      </motion.div>
    </div>
